...
interface EditorGridProps {
  ...
  unreachableCells?: Set<string>;
  unreachableGoal?: boolean;
}
...
const GoalMarker: React.FC<{ unreachable?: boolean }> = ({ unreachable }) => (
  <div className="absolute inset-0 flex items-center justify-center pointer-events-none animate-pulse z-10">
    <div className={`w-4/5 h-4/5 rounded-lg flex items-center justify-center shadow-md ${
      unreachable
        ? 'bg-gradient-to-br from-red-300 to-red-500 ring-2 ring-red-500 ring-offset-1 animate-pulse'
        : 'bg-gradient-to-br from-green-300 to-emerald-500'
    }`}>
      <span className="text-xl">{unreachable ? '❌' : '🏁'}</span>
    </div>
  </div>
);

const StarMarker: React.FC<{ unreachable?: boolean }> = ({ unreachable }) => (
  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 animate-bounce-slow">
    <svg width="60%" height="60%" viewBox="0 0 24 24" className={unreachable ? 'opacity-50 grayscale' : 'drop-shadow-md'}>
      <defs>
        <linearGradient id="editorStarGrad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor={unreachable ? '#9ca3af' : '#fde047'} />
          <stop offset="100%" stopColor={unreachable ? '#6b7280' : '#eab308'} />
        </linearGradient>
      </defs>
      <path
        d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"
        fill="url(#editorStarGrad)"
        stroke={unreachable ? '#4b5563' : '#ca8a04'}
        strokeWidth="0.5"
      />
    </svg>
  </div>
...
export const EditorGrid: React.FC<EditorGridProps> = ({
  ...
  const starMap = useMemo(() => {
    ...
    stars.forEach((s) => map.set(`${s.x},${s.y}`, s));
    return map;
  }, [stars]);

  const unreachableStarSet = useMemo(() => {
    const set = new Set<string>();
    if (unreachableCells) {
      stars.forEach((s) => {
        if (unreachableCells.has(`${s.x},${s.y}`)) {
          set.add(`${s.x},${s.y}`);
        }
      });
    }
    return set;
  }, [stars, unreachableCells]);
      ...
      >
        {grid.map((row, y) =>
          row.map((cell, x) => {
            ...
            const cellKey = `${x},${y}`;
            const hasStar = starMap.has(cellKey);
            const isStarUnreachable = unreachableStarSet.has(cellKey);
            const isCellUnreachable = unreachableCells &&
              cell !== 'wall' &&
              !isStart &&
              !isGoal &&
              !unreachableCells.has(cellKey);
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isCellUnreachable ? 'bg-red-50/80' : ''}
                `}
                style={{
                  left: x * cellSize,
                  top: y * cellSize,
              ...
              >
                ...
                {isGoal && <GoalMarker unreachable={unreachableGoal} />}
                {hasStar && <StarMarker unreachable={isStarUnreachable} />}

                {isStarUnreachable && (
                  <div className="absolute top-0 right-0 z-30">
                    <span className="text-xs bg-red-500 text-white rounded-bl px-1 leading-tight">!</span>
                  </div>
                )}
              </div>
            );
